...
import { ArrowLeft, ArrowRight, Check, Loader2, Calculator, Layers, ClipboardList, CreditCard, Square } from "lucide-react";
...
export default function NovoOrcamento() {
  ...
  const {
    ...
    unidadeOriginal, totais, precoFinal, variaveisCompletas,
    categorias, tipologiasDaCategoria,
    loadingTipologiaCompleta,
    tiposVidroDisponiveis, itensConfiguracao,
    ...
    <div className="min-h-screen bg-slate-50/50">
      ...
      <div className="w-full px-4 py-8">
        <AnimatePresence mode="wait">
          ...
          {/* Etapa 2: Tipologia + Variáveis + Vidro */}
          {etapaAtual === 2 && (
            <motion.div key="tipologia-variaveis" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              {!tipologiaSelecionada ? (
                <>
                  <div className="mb-6">
                    <h2 className="text-2xl font-bold text-slate-900">{categoriaSelecionada?.nome || 'Tipologias'}</h2>
                    <p className="text-slate-500 mt-1">Escolha o modelo específico</p>
                  </div>
                  {tipologiasDaCategoria.length === 0 ? (
                    <Card className="text-center py-12"><CardContent><ClipboardList className="w-12 h-12 mx-auto text-slate-300 mb-3" /><p className="text-slate-500">Nenhuma tipologia disponível</p></CardContent></Card>
                  ) : (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3">
                      {tipologiasDaCategoria.map((tip, i) => (
                        <TipologiaCard key={tip.id} tipologia={tip} onClick={() => selecionarTipologia(tip)} index={i} />
                      ))}
                    </div>
                  )}
                </>
                ...
                <>
                  ...
                  <Card className="mb-6 bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200">
                    <CardContent className="p-4 sm:p-6">
                      <div className="flex flex-row items-start gap-4">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => { setTipologiaSelecionada(null); setTipoVidroSelecionado(null); }} className="text-slate-500">
                          Alterar
                        </Button>
                      </div>
                    </CardContent>
                  ...
                  <div className="space-y-6">
                    <Card>
                      <CardHeader><CardTitle className="text-lg">Variáveis de Entrada</CardTitle></CardHeader>
                    ...
                    {variaveisCompletas && (
                      <Card>
                        <CardHeader>
                          ...
                          <p className="text-sm text-slate-500 mt-1">Selecione a cor e veja o preço final estimado</p>
                        </CardHeader>
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <div className="w-10 h-10 rounded-lg border-2 border-white shadow-sm" style={{ backgroundColor: tipo.cor || '#e2e8f0' }} />
                                  <div>
              ...
              <div className="w-full max-w-2xl mx-auto">
                {pecasCalculadas.length > 0 && (
                  <PecaConferencia
                    ...
                    onConfirmar={() => confirmarPeca(pecaConferenciaAtual, 4)}
          ...
          {etapaAtual === 4 && (
            <motion.div key="resumo" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="grid lg:grid-cols-2 gap-8">
                <Card>
                  <CardHeader><CardTitle className="text-lg">Peças Calculadas</CardTitle></CardHeader>
                  <CardContent className="space-y-3">
                ...
                <div className="space-y-6">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        <Label>Nome</Label>
                        <Input value={clienteInfo.nome} onChange={(e) => setClienteInfo({ ...clienteInfo, nome: e.target.value })} placeholder="Nome do cliente" className="mt-1" />
                      </div>
                      <div>
                        <Label>Telefone</Label>
                        <Input value={clienteInfo.telefone} onChange={(e) => setClienteInfo({ ...clienteInfo, telefone: e.target.value })} placeholder="(00) 00000-0000" className="mt-1" />
                      </div>
                      <div>
                        <Label>E-mail</Label>
                        <Input value={clienteInfo.email} onChange={(e) => setClienteInfo({ ...clienteInfo, email: e.target.value })} placeholder="email@exemplo.com" className="mt-1" />
                      </div>
                    </CardContent>
                  </Card>

                  <Button onClick={salvarOrcamento} disabled={salvarMutation.isPending} className="w-full h-14 bg-green-600 hover:bg-green-700 text-lg font-medium">
                    {salvarMutation.isPending ? (
                      <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Salvando...</>
                    ) : (
                      <><Check className="w-5 h-5 mr-2" /> Finalizar Orçamento</>
                    )}
                  </Button>
                </div>
              </div>
